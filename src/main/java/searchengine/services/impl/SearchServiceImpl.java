...
public class SearchServiceImpl implements SearchService {
    ...
    public ResponseEntity<Object> search(String query, String site, Integer offset, Integer limit) throws IOException {
        ...
        lemmasForSearch.removeIf(e -> {
            Integer lemmaFrequency = lemmaRepository.findCountPageByLemma(Integer.valueOf(e.getLemma()), e.getSiteId());
        ...
        Map<Integer, IndexSearch> indexesByLemmas = indexRepository.findIndexByLemma(sortedLemmasToSearch.get(0).getId()).stream().collect(Collectors.toMap(IndexSearch::getPageId, index -> index));
        for(int i = 0; i <= sortedLemmasToSearch.size() - 1; i++){
            List<IndexSearch> indexNextLemma = indexRepository.findIndexByLemma(sortedLemmasToSearch.get(i).getId());
        ...
        for(RankDto rank : pagesRelevanceSorted){
            ...
            for(String sentence : sentences){
                ...
                for(String word : words){
                    String lemmasFromWord = lemmaService.getLemmaByWord(word.replaceAll("\\p{Punct}",""));
                    if(simpleLemmasFromSearch.contains(lemmasFromWord)){
        ...
        List<SearchDataResponse> sortedSearchDataResponses = searchDataResponses.stream().sorted(Comparator.comparingDouble(SearchDataResponse::getRelevance).reversed()).toList();
        List<SearchDataResponse> result = new ArrayList<>();
        for (int i = limit * offset; i <= limit * offset + limit; i++) {
            try{
                result.add(sortedSearchDataResponses.get(i));
    ...
    private void markWord(StringBuilder textFromElement, String word, int startPosition){
        int start = textFromElement.toString().indexOf(word);
