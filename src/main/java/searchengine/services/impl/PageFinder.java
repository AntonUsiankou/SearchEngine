...
public class PageFinder extends RecursiveAction {
    ...
    public PageFinder(SiteRepository siteRepository,
                      PageRepository pageRepository,
                      SitePage siteDomain,
                      String page,
                      ConcurrentHashMap<String, Page> resultForkJoinPoolIndexedPages,
                      Connection connection,
                      LemmaService lemmasService,
    ...
    protected void compute() {
        ...
        try{
            ...
            if (indexingPage.getContent() == null|| indexingPage.getContent().isEmpty() || indexingPage.getContent().isBlank()) {
                throw new Exception("Content of side id:" + indexingPage.getSiteId() + ", page:" + indexingPage.getPath() + " is null or empty");
            }
        ...
        for(String url : urlSet) {
            if(resultForkJoinPoolIndexedPages.get(page) != null || !indexingProcessing.get()) {
                PageFinder task = new PageFinder(siteRepository, pageRepository, sitePage, url, resultForkJoinPoolIndexedPages, connection, lemmasService, pageIndexerService, indexingProcessing);
                task.fork();
    ...
    public void refreshPage(){
        ...
        if(pageToRefresh != null) {
            ...
            pageIndexerService.indexHtml(pageToRefresh.getContent(), pageToRefresh);
        } else {
            pageRepository.save(indexingPage);
